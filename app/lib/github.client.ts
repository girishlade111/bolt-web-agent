...
export async function startDeviceFlow(): Promise<DeviceFlowStart> {
  ...
  if (!res.ok) {
    throw new Error(data.error ?? `Device flow failed to start: ${res.status}`);
  }
...
export async function connectGitHub(
  ...
  while (Date.now() < deadline) {
    ...
    if (result.status === 'slow_down') {
      interval += 5000;
    } // honor the github slow_down guidance

    if (result.status === 'expired') {
      throw new Error('Device code expired — try connecting again');
    }
...
export function collectFiles(): Record<string, string> {
  ...
  for (const [absPath, dirent] of Object.entries(files)) {
    ...
    if (dirent.isBinary) {
      continue;
    } // skip binary for now (isBinary content is '')
