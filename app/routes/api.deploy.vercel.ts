...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  if (!token) {
    return json({ error: 'Missing Vercel token' }, { status: 401, headers });
  }
  ...
  try {
    ...
    if (!depRes.ok) {
      return json(
        { error: dep?.error?.message ?? `Vercel status check failed: ${depRes.status}` },
        { status: 502, headers },
      );
    }

    const readyState = String(dep.readyState ?? dep.state ?? 'QUEUED');
    const vercelUrl = dep.url
      ? String(dep.url).startsWith('http')
        ? dep.url
        : `https://${dep.url}`
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  if (!token) {
    token = (await getStoredToken(env, 'vercel', sessionId!)) ?? undefined;
  }
  ...
  if (!token) {
    return json(
      { error: 'Missing Vercel token. Create at https://vercel.com/account/tokens (scope: deployment)' },
  ...
  if (!token.startsWith('vercel_') && token.length < 20) {
    throw json(
      { error: 'Vercel token invalid: should start with vercel_ or be a valid PAT' },
      { status: 401, headers },
    );
  }
  ...
  try {
    ...
    const vercelUrl = vercelData.url
      ? String(vercelData.url).startsWith('http')
        ? vercelData.url
        : `https://${vercelData.url}`
      : `https://${projectName}.vercel.app`;
    const status =
      readyState === 'READY' ? 'ready' : readyState === 'ERROR' || readyState === 'CANCELED' ? 'error' : 'initializing';
