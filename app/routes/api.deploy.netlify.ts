...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  if (!token) return json({ error: 'Missing Netlify token' }, { status: 401, headers });

  // deploymentId format: siteId:deployId (set by the action)
  const [siteId, deployId] = String(deploymentId).split(':');
  if (!siteId || !deployId) return json({ error: 'Invalid deploymentId' }, { status: 400, headers });
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    // Validate token first for a clear error message
    const userRes = await fetch('https://api.netlify.com/api/v1/user', { headers: { Authorization: `Bearer ${token}` } });
    if (!userRes.ok) {
      const txt = await userRes.text().catch(() => '');
      throw json({ error: `Netlify token invalid: ${userRes.status} ${txt.slice(0,200)}` }, { status: 401, headers });
    ...
    if (!deployRes.ok) {
      throw json({ error: deploy?.message ?? `Netlify deploy creation failed: ${deployRes.status}` }, { status: 502, headers });
    }
